...
@section('content')
                    ...
                    <table id="example" class="table table-striped table-bordered w-100 mt-3">
                        ...
                        <tbody>
                            @foreach ($data as $index => $datas)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ ucwords($datas->instansi) }}</td>
                                    <td>{{ Str::limit($datas->alamat, 50) ?? '-' }}</td>
                                    <td>{{ $datas->email ?? '-' }}</td>
                                    <td>{{ $datas->no_hp ?? '-' }}</td>
                                    <td>{{ ucwords($datas->kategori) ?? '-' }}</td>
                                    @if (Auth::user()->role == 'superadmin' || Auth::user()->role == 'admin')
                                        <td class="d-flex flex-column flex-sm-row">
                                            ...
                                            <a href="{{ route('instansi.destroy', $datas->id) }}"
    ...
    @include('superadmin.instansi.add')
    @include('superadmin.instansi.edit')
    @push('style')
        {{-- datatable --}}
    ...
    @push('scripts')
        ...
        <script>
            ...
            function printPDF() {
                ...
                let title = "Data Outlet E-REPORTING PT SOLAS LANGGENG SEJAHTERA";
            ...
            function exportExcel() {
                ...
                XLSX.writeFile(wb, "Data_outlet.xlsx");
