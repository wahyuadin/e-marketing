...
@section('content')
                            ...
                            @forelse ($data as $index => $datas)
                                <tr>
                                    ...
                                    <td>
                                        <ul>
                                            @php
                                                $instansiDetails = $datas->instansiDetail ?? collect();
                                            @endphp

                                            @foreach ($instansiDetails->take(2) as $instansiDetail)
                                                @php
                                                    $dokter = optional($instansiDetail->dokter);
                                                    $spesialis = optional($dokter->spesialis);
                                                @endphp
                                                <li>{{ $dokter->dokter ?? '' }} - {{ $spesialis->spesialis ?? '' }}</li>
                                            @endforeach

                                            @if ($instansiDetails->count() > 2)
                                                <li>...</li>
                                            @elseif ($instansiDetails->isEmpty())
                                    ...
                                    <td class="d-flex flex-column flex-sm-row gap-2">
                                        ...
                                        <!-- Tombol Detail -->
                                        <a href="{{ route('instansi-data.show', $datas->id) }}"
                                            class="btn btn-info btn-sm bx bx-detail" title="Detail">
                                        </a>

                                        <!-- Tombol Hapus -->
                                        <a href="{{ route('instansi-data.destroy', $datas->id) }}"
    ...
    @include('admin.instansi.add')
    @include('admin.instansi.edit')
    @push('style')
        {{-- datatable --}}
