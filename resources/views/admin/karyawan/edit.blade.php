...
<div class="modal fade" id="editData{{ $datas->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <form action="{{ route('karyawan.update', $datas->id) }}" method="POST" class="needs-validation" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="nama_karyawan" id="karyawan" required
                                    value="{{ old('karyawan', $datas->nama_karyawan) }}" placeholder="Masukan Nama Karyawan">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select name="jabatan_id" class="form-select">
                                    ...
                                    @foreach ($jabatan as $jabatans)
                                        <option value="{{ $jabatans->id }}" {{ $jabatans->id == $datas->jabatan_id ? 'selected' : '' }}>{{ $jabatans->jabatan }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
