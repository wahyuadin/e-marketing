<div class="modal-content">
    <form action="{{ route('sales.area-manager.update', $data->id) }}" method="POST" class="needs-validation" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                ...
                <div class="col-md-12">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="nama_area_manager" id="nama_area_manager" required
                            value="{{ old('nama_area_manager', $data->nama_area_manager) }}" placeholder="Masukan Nama Area Manager">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select name="regional_id" id="regional_id" class="form-select" required>
                            ...
                            @foreach ($regional as $item)
                                <option value="{{ $item->id }}" {{ $item->id == $data->regional_id ? 'selected' : '' }}>{{ $item->nama_regional }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        ...
                        <select name="jabatan_id" id="jabatan_id" class="form-select" required>
                            ...
                            @foreach ($jabatan as $item)
                                <option value="{{ $item->id }}" {{ $item->id == old('jabatan_id', $data->jabatan_id) ? 'selected' : '' }}>{{ $item->jabatan }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="kode_area_manager" id="kode_area_manager" required
                            value="{{ old('kode_area_manager', $data->kode_area_manager) }}" placeholder="Masukan Kode MR">
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="wilayah" id="wilayah" required
                            value="{{ old('wilayah', $data->wilayah) }}" placeholder="Masukan Wilayah">
                    </div>
                </div>
            </div>
        </div>
